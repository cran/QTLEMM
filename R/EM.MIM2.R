...
#' is an n*g matrix, where n is the number of individuals which are
#' genotyped, and g is the number of possible genotypes of QTLs. If
#' cp.matrix=NULL, the function will calculate the conditional
#' probability matrix for selective genotyping.
...
EM.MIM2 <- function(QTL, marker, geno, D.matrix, cp.matrix = NULL, y, yu = NULL,
  ...
  if(sele.g == "p" | sele.g == "f"){
    ...
    QTL.p <- function(QTL, marker, geno, D.matrix, ys, yu, cM = TRUE, type = "RI", ng = 2,
                      ...
                      conv = 10^-5, console = FALSE, pf = FALSE){
      y <- c(ys,yu)
      N <- length(y)
      nu <- length(yu)
      nQTL <- nrow(QTL)
